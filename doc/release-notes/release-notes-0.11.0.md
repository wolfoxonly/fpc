Flashpaychain Core version 0.11.0 is now available from:

  <https://Flashpaychain.org/bin/Flashpaychain-core-0.11.0/>
...
For example, add the following to `Flashpaychain.conf`:
...
related to the blockchain in the Flashpaychain system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Flashpaychain Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Flashpaychain Core will 
...
For an example script see [Flashpaychain-submittx](https://github.com/laanwj/Flashpaychain-submittx).
...
- #5334 `cf87536` libFlashpaychainconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new Flashpaychain-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only FlashpaychainAmountField
- #5753 `0f44672` Add Flashpaychain logo to about screen
...
- #5809 `46bfbe7` Add Flashpaychain-cli man page
...
- #5849 `41259ca` contrib/init/Flashpaychaind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Flashpaychain-Qt.app to Flashpaychain-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for Flashpaychain-tx)
