...
The RPM spec file provided here is for Flashpaychain-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://Flashpaychain.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Flashpaychain git repository](https://github.com/Flashpaychain/Flashpaychain) but are not part of
...
Currently only `Source10` is used, for the example `Flashpaychain.conf` file.
...
in the Flashpaychain git repository. At present only an SVG version of the Flashpaychain
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the Flashpaychain
...
needed when 0.12.1 is released, a proper fix is already in the Flashpaychain git
master branch.
...
idea to build Flashpaychain Core against a static version of that library compiled
...
The `Flashpaychain.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `Flashpaychain-server` package. At this point, most RPM based
...
The `Flashpaychain-server` package should have SELinux support. How to properly do
...
Flashpaychain-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/Flashpaychain/). He has been
packaging Flashpaychain for Fedora at least since 2012.
...
4. I renamed the `Flashpaychain` package that contains the Qt GUI to `Flashpaychain-core` as
that appears to be how the general population refers to it, in contrast to
`Flashpaychain-xt` or `Flashpaychain-classic`. I wanted to make sure the general population
