#!/bin/bash
# Copyright (c) 2013 The Flashpaychain Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Flashpaychain database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Flashpaychain datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Flashpaychain datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Flashpaychain 0.7 datadir."
    ;;
  3)
    echo "Detected Flashpaychain pre-0.8 datadir."
    ;;
  4)
    echo "Detected Flashpaychain 0.8 datadir."
    ;;
esac
