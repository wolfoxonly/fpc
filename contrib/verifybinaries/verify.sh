...
###   Flashpaychaincore.org and Flashpaychain.org and compares them.
...
WORKINGDIR="/tmp/Flashpaychain_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://Flashpaychaincore.org"
HOST2="https://Flashpaychain.org"
BASEDIR="/bin/"
VERSIONPREFIX="Flashpaychain-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'Flashpaychain-',
...
if [ $? -ne 0 ]; then
   echo "Flashpaychain.org failed to provide signature file, but Flashpaychaincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "Flashpaychain.org and Flashpaychaincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Flashpaychain Core binary release signing key installed?"
   fi
