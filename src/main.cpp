...
// Copyright (c) 2011-2015 The Flashpaychain developers
...
const string strMessageMagic = "Flashpaychain Signed Message:\n";
...
int64 GetProofOfStakeReward(int64 nCoinAge,int64 balance)//<zxb>
{
    ...
    int year =  std::ceil(pindexBest->nHeight / (24 * 60 / 2 * 365));//zxb fpc利率衰减部分
    printf("year ===== %d\n", year);
...
void ThreadScriptCheck() {
    RenameThread("Flashpaychain-scriptch");
...
bool CBlock::ConnectBlock(CValidationState &state, CBlockIndex* pindex, CCoinsViewCache &view, bool fJustCheck)
{
    ...
    // ppcoin: fees are not collected by miners as in Flashpaychain
...
bool LoadBlockIndex()
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsLimit =%d,nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "Flashpaychain", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnProofOfWorkLimit.GetCompact(),bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
...
// ~uint256(0) >> 32
    ...
    if (!fReindex) {
      ...
      //  block.nNonce   = 2179302059u;
        block.nNonce = ByteReverse(8818506);//<zxb>fpc
        if (fTestNet)
        {
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("FlashpaychainMiner : proof-of-work not meeting target");

    //// debug print
    printf("FlashpaychainMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FlashpaychainMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FlashpaychainMiner : ProcessBlock, block not accepted");
    }
...
#ifdef TESTING
void FlashpaychainMiner(CWallet *pwallet, bool fProofOfStake, bool fGenerateSingleBlock)
#else
void FlashpaychainMiner(CWallet *pwallet, bool fProofOfStake)
#endif
{
    ...
    {
        if(!fProofOfStake) printf("FlashpaychainMiner terminated\n");
        throw;
    }
}

void GenerateFlashpaychains(bool fGenerate, CWallet* pwallet)
{
...
#ifdef TESTING
        minerThreads->create_thread(boost::bind(&FlashpaychainMiner, pwallet, false, false));
#else
        minerThreads->create_thread(boost::bind(&FlashpaychainMiner, pwallet, false));
#endif
}
...
void static ThreadStakeMinter(void* parg)
{
    ...
    try
    {
        FlashpaychainMiner(pwallet, true);
    }
    catch (boost::thread_interrupted) {
