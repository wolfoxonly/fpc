...
void CWalletTx::RelayWalletTransaction()
{
    ...
    {
        // Important: versions of Flashpaychain before 0.8.6 had a bug that inserted
            ...
            loop
            {
                ...
                if (nChange > 0)
                {
                    ...
                    else if (!GetBoolArg("-avatar")) // ppcoin: not avatar mode; nu: avatar mode enabled by default only on Share wallet to avoid change being sent to hidden address
                    {
                        ...
                        // change transaction isn't always pay-to-Flashpaychain-address
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WriteName(CFlashpaychainAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).EraseName(CFlashpaychainAddress(address).ToString());
}
