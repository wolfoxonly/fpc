...
#else
    const char* pszModule = "Flashpaychain";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Flashpaychain
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Flashpaychain
    // Mac: ~/Library/Application Support/Flashpaychain
    // Unix: ~/.Flashpaychain
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Flashpaychain";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Flashpaychain.conf"));
    ...
    if (mapArgs.count("-conf") == 0 && !boost::filesystem::exists(pathConfigFile)) {
        ...
        else
        {
            ...
            try {
                const char* content = "rpcuser=Flashpaychainrpc\nrpcpassword=Flashpaychainrpcpassword";
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No Flashpaychain.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override Flashpaychain.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Flashpaychaind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Flashpaychain will not work properly.");
...
// Format the subversion field according to BIP 14 spec (https://en.Flashpaychain.it/wiki/BIP_0014)
std::string FormatSubVersion(const std::string& name, int nClientVersion, const std::vector<std::string>& comments)
{
    ...
    ss << "/";
    ss << "Flashpaychain:" << FormatVersion(FLASHPAYCHAIN_VERSION);
    ss << "(" << CLIENT_BUILD << ")/";
    return ss.str();
}
