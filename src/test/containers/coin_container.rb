...
class CoinContainer
  def create(options = {})
    ...
    bash_cmd += "./Flashpaychaind " + cmd_args.join(" ")

    if options[:remove_addr_after_shutdown]
      bash_cmd += "; rm -f /root/.Flashpaychain/testnet/peers.dat"
    end

    if options[:remove_wallet_after_shutdown]
      bash_cmd += "; rm /root/.Flashpaychain/testnet/wallet.dat"
    end
